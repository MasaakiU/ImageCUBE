...
class AddedContent_Spectrum(AddedContent):
    ...
    def export_spectrum(self):
        self.parent_window.spectrum_widget.save_spectrum_as_spc(self.item)
...
class AddedContent_Map(AddedContent):
    ...
    def focus_unfocus(self, focused):
        ...
        if self.focused:
            ...
            self.parent_window.map_widget.conceal_map(self.item)
...
class ToolbarLayout(QVBoxLayout):
    def __init__(self, window_type, parent=None):
        self.parent = parent
        self.isGradient = False # コントラスト調整グラジエントがあるかどうか
    ...
    # def execute_curve_fitting(self):
    #     # 範囲を決める
    #     range_settings_popup = popups.RangeSettingsPopup(parent=self.parent, initial_values=(295, 360), title="curve fitting (gaussian)")
    #     done = range_settings_popup.exec_()
    #     if done == 1:
    #         RS1 = range_settings_popup.spbx_RS1.value()
    #         RS2 = range_settings_popup.spbx_RS2.value()
    #         RS1, RS2 = np.sort([RS1, RS2])
    #         image2D = self.parent.spectrum_widget.curve_fitting(RS1, RS2)
    #         # ボタン追加
    #         ID = self.added_map_layout.count()
    #         self.add_content(image2D, "added_map", option="%d\n%s"%(ID+1, image2D.name))
    # アンミックス
    ...
    # # 画像計算
    # def launch_image_calculator(self):
    #     Image2D_list = self.parent.map_widget.image2D_list
    #     if len(Image2D_list) == 0:
    #         warning_popup = popups.WarningPopup("There are no generated images!")
    #         warning_popup.exec_()
    #     else:
    #         image_calculator = popups.ImageCalculator(Image2D_list, parent=self.parent)
    #         done = image_calculator.exec_()
    #         # OKされたとき
    #         if done == 1:
    #             new_image2D = image_calculator.created_image2D
    #             self.add_content(new_image2D, "added_map", option="\n%s"%new_image2D.name)
    #             self.parent.map_widget.display_map(new_image2D)
    ...
    def set_CellFreePosition(self):
        # イメージが少なくとも1枚は開いてあることを前提としよう（見境なくバックグラウンド決められても困る）
        if not len(self.parent.map_widget.image2D_list):
            warning_popup = popups.WarningPopup("at least one image should be reconstructed before setting cell-free position")
            warning_popup.exec_()
            return False
        else:
    ...
    def execute_spectrum_linear_subtraction(self):
        ...
        #     # スペクトル描画
        #     plot_data_item = self.parent.spectrum_widget.add_calculated_spectrum()
        #     # 一つだけ存在するはずのadded_spectrumの名前を取得
        #     added_spectrum_name = os.path.splitext(os.path.basename(self.added_spectrum_layout.itemAt(0).widget().toolTip().split("\n")[1]))[0]
        #     # ボタン追加
        #     ID = self.added_map_layout.count()
        #     self.add_content(plot_data_item, "subtracted_spectrum", option="\n%s_minus_%s_%d-%d"%(self.parent.file_name_wo_ext, added_spectrum_name, sRS, eRS))

            # 保存：保存先は同じディレクトリ（順次番号をつけてく感じにしよう）
...
class SpectrumWidget(pg.PlotWidget):
    ...
    def display_map_spectrum(self):
        ...
        # 引き算タイプのスペクトル
            ...
            value_list = self.height_list_set2[sub_idx]
            for spct_idx, calculated_spectrum in enumerate(self.full_y_matrix2 * value_list):
                self.calculatedSpectrum_list2[spct_idx].setData(self.master_x_list2, y_list[np.argsort(self.spc_file.x)] - calculated_spectrum)
            return
    ...
    # spc形式でスペクトルを保存
    def save_spectrum_as_spc(self, item):
    ...
    def spectrum_linear_subtraction(self, sRS, eRS):
        ...
        x_min = max([x_min, min(added_x_list)])
        x_max = min([x_max, max(added_x_list)])
        common_x_region = (x_min <= self.spc_file.x) & (self.spc_file.x <= x_max)
        umx_x_list = self.spc_file.x[common_x_region]
        # [:, 0]は追加スペクトル、[:, 1]はオリジナル
        umx_y_matrix = np.empty((len(umx_x_list), 2), dtype=float, order="F")
        umx_y_matrix[:, 0] = np.interp(umx_x_list, added_x_list, added_y_list)
        umx_y_matrix[:, 1] = self.spc_file.sub[0].y[common_x_region]
        # 大元（fittingされる前のもの）は、spc_fileから情報を取得しよう（そのほうが楽だし…）
        master_x_list, master_y_list = self.spc_file.get_data(sRS, eRS, sub_idx=0)
        # 補間して揃える
        added_regional_y_list = np.interp(master_x_list, added_x_list, added_y_list)
        # 最小二乗
        umx_height_matrix = gf.spectrum_linear_subtraction_core(master_x_list, master_y_list, added_regional_y_list)
        # 追加
        plot_data_item = pg.PlotDataItem(umx_x_list, (umx_y_matrix * umx_height_matrix).sum(axis=1), fillLevel=0, pen=gf.mk_u_pen())
...
class MapWidget(pg.GraphicsLayoutWidget):
    def __init__(self, spc_file=None, parent=None):
        self.parent = parent
        self.image2D_list = []
        self.cur_image2D_idx = None
        self.ContrastConfigurator = None
        super().__init__()
    ...
    def initiate_map_spectrum(self, spc_file):
        pass
        # もし何か追加するようなら、ボタンも追加するのを忘れずに〜
    # Image2D インスタンスを渡された場合
    def display_map(self, image2D):
        # そもそもイメージがまだ無ければ、イメージ及びグラジエントを追加する
        if not self.parent.toolbar_layout.isGradient:
            # グラジエント
            ...
            # グラジエント追加しましたよ宣言
            self.parent.toolbar_layout.isGradient = True
        # 新しく追加された場合はリストに追加
        if not image2D in self.image2D_list:
            self.image2D_list.append(image2D)
        # 既に追加されているImageItemを変更する形で、画像を表示
        self.cur_image2D_idx = self.image2D_list.index(image2D)
        ...
        return image2D
    # 隠す
    def conceal_map(self, image2D):
        self.map_img.hide()
    # 除く
    def removeImage2D(self, image2D):
        self.image2D_list.remove(image2D)
        cur_N_image2D = len(self.image2D_list)
        if cur_N_image2D > 0:
            # 現在のimage2D_idxを表示
            self.cur_image2D_idx = min(self.cur_image2D_idx, cur_N_image2D - 1)
            self.display_map(self.image2D_list[self.cur_image2D_idx])
        # 1つしかなかった場合は、何も表示しなくする
        else:
            self.clear_all()
        self.img_view_box.updateViewLists()
    # ひとまずimageを開いていない状態にする
    def clear_all(self):
        self.map_img.image = None
        self.map_img.qimage = None
        self.paintMode = None
        self.lut = None
        self.h_crosshair.hide()
        self.v_crosshair.hide()
        self.cur_image2D_idx = None
        self.text_item.setText("x=0, y=0, value=0")
        self.ContrastConfigurator.setParent(None)
        self.parent.toolbar_layout.isGradient = False
    def on_click(self, event):
        if event.button() == Qt.LeftButton:
            event.accept()
            # クリックしたときのview boxを取得
            if len(self.image2D_list) != 0:
    ...
    def get_map_xy_fm_QpointF(self, loc_of_QPointF):
        ...
        cur_image2D = self.image2D_list[self.cur_image2D_idx]
        # print(map_x, map_y, cur_image2D.x, cur_image2D.y, (0 <= map_x < cur_image2D.x) and (0 <= map_y < cur_image2D.y))
        if (0 <= map_x < cur_image2D.x) and (0 <= map_y < cur_image2D.y):
            value = np.round(cur_image2D.image2d[map_x, map_y], 3)
    ...
    def get_cur_image2D(self):
        try:
            return self.image2D_list[self.cur_image2D_idx]
        except:
            return None
